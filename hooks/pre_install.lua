...
--- @param ctx table Context with ctx.version string (user-input version, may be a prefix or "latest")
--- @return table Version information
function PLUGIN:PreInstall(ctx)
    local version = ctx.version

    -- "latest" → resolve to the newest available version
    ...
    -- Collect all available versions and their download URLs
    local available = {}
    selector_div:find("option"):each(function(_i, option)
        local opt_version = option:text()
        local opt_url = option:attr("value")
        if type(opt_version) == "string" and opt_version ~= "" and opt_version:match("%d") then
            available[opt_version] = opt_url
        end
    end)

    -- Resolve: exact match first, then prefix/latest
    local resolved_version = (version ~= "" and available[version]) and version or util.findBestVersion(available, version)
    local resolved_url = resolved_version and available[resolved_version]

    if not resolved_version then
        error("Could not find Grails version matching: " .. (version == "" and "latest" or version))
    end
    ...
    if resolved_url and resolved_url:find("apache%.org") then
        local checksum = fetchApacheChecksum(resolved_url)
        if checksum then
